...
public class DepthFirstSearch extends SearchAlgorithm{
    ...
    public boolean solveMaze() {
        stack.add(maze.getMaze()[0][0]);

        while (!stack.isEmpty()) {
            Node tmp = stack.pop();
            tmp.setVisited(true);
            expanded++;
            maxFringe = Math.max(maxFringe, stack.size());
            int row = tmp.getRow();
            int col = tmp.getCol();
            ...
            if (isValid(row + 1, col)) {
                ...
                stack.push(maze.getMaze()[row + 1][col]);
            }

            if (isValid(row, col + 1)) {
                maze.getMaze()[row][col + 1].setPrev(tmp);
                stack.push(maze.getMaze()[row][col + 1]);
            }
...
//            System.out.println("false\n\n");
    ...
    private boolean isValid(int i, int j) {
        if (i >= 0 && i < maze.getDim() && j >= 0 && j < maze.getDim() && !maze.getMaze()[i][j].isOccupied() && !maze.getMaze()[i][j].isVisited() && !stack.contains(maze.getMaze()[i][j])) {
            return true;
        } else {
            return false;
        }
